...
export function InvoicesTable() {
  ...
  const confirmBulkDelete = async () => {
    if (!bulkIds) return
    setBulkBusy(true)
    const results = await Promise.allSettled(bulkIds.map((id) => deleteInvoice.mutateAsync(id)))
    const ok = results.filter((r) => r.status === 'fulfilled').length
    const failed = results.length - ok
    setBulkBusy(false)
    setBulkIds(null)
    selection.clear()
    if (failed === 0) toast.success('Deleted', `${ok} invoice${ok === 1 ? '' : 's'} deleted.`)
  ...
  const onExportCsv = async () => {
    setExporting(true)
    try {
      const all = await invoicesApi.list({ ...query, page: 1, pageSize: 1000 })
      downloadCsv(
        'invoices.csv',
        ['Number', 'Customer', 'Status', 'Amount', 'Currency', 'Due', 'Paid'],
        all.data.map((i) => [
          i.number,
          customerName(i.customerId),
          i.status,
          amountFromCents(i.amountCents),
          i.currency,
          i.dueDate,
          i.paidAt ?? '',
        ])
      )
      toast.success('Export ready', `${all.data.length} invoices exported.`)
    } catch {
      toast.error('Export failed', 'Could not export invoices.')
    } finally {
      setExporting(false)
    }
  }
  ...
  const columns: Column<Invoice>[] = [
    ...
    },
    {
      header: 'Paid',
      className: 'whitespace-nowrap',
      render: (i) => <span className="text-muted-foreground">{i.paidAt ? formatDate(i.paidAt) : '—'}</span>,
    },
  ]
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            Delete <span className="font-medium text-foreground">{deleting?.number}</span>? This cannot be undone.
          </>
        }
      ...
      <ConfirmDialog
        ...
        description={`Delete ${bulkIds?.length ?? 0} selected invoice${bulkIds?.length === 1 ? '' : 's'}? This cannot be undone.`}
        confirmLabel="Delete"
        loading={bulkBusy}
      />
    </>
